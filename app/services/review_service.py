...
def create_user_profile(
    ...
    profile_prompt = ChatPromptTemplate.from_messages([
        ("system", '''あなたは優秀なデータアナリストです。
            ...
            - **お酒以外のレビューの扱い**: レビュー履歴にはお酒以外の製品（例: 食品、ジュース）のレビューも含まれることがあります。これらは基本的にお酒の好みとは無関係なため、無視してください。ただし、そのレビューがお酒の好みを推測する上で明らかに役立つ情報（例: 「甘いものが好きなので、このジュースもとても美味しい」）を含んでいる場合に限り、補助的な情報として考慮しても構いません。
            ...
            2.  **入力**:
                ...
                **正しい判断**:
                - `taste1`: "甘口" (特別な日に飲みたい、というポジティブな評価から、辛口や苦みよりは甘口寄りだと推測)
                - `taste2`: ["酸味が穏やか"]
                - `aroma_preference`: ["芳醇"]
            ...
            3. **入力**:
                ...
                **正しい判断**: このレビューはお酒ではないため、プロファイルに直接反映しません。ただし、「酸味」が好みである可能性を補助情報として記憶し、他のお酒レビューで同様の傾向が見られる場合に`taste2`に「酸味」を含めることを検討します。
            '''
        ),
        ("user", "レビュー履歴:\n{history}")
    ])
